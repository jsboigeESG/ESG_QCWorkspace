...
class OptimizedSwingTradingCryptoAlgorithm(QCAlgorithm):
    def Initialize(self):
        ...
        self.crypto_symbols = [
            ...
            self.AddCrypto("USDTUSD", Resolution.Daily, Market.GDAX).Symbol,
        ...
        self.max_risk_per_trade = 0.01  # Réduction du risque à 1% par trade
        ...
        self.max_open_positions = 2  # Limiter le nombre de positions ouvertes simultanément
        self.max_drawdown = 0.25  # Drawdown maximum autorisé de 25%

    def OnData(self, data):
        # Vérifiez le drawdown actuel
        current_drawdown = (self.starting_portfolio_value - self.Portfolio.TotalPortfolioValue) / self.starting_portfolio_value
        if current_drawdown > self.max_drawdown:
            self.Liquidate()
            self.Debug(f"Liquidation totale due au drawdown de {current_drawdown:.2%}")
            return

        open_positions = sum(1 for symbol in self.crypto_symbols if self.Portfolio[symbol].Invested)
        ...
        for symbol in self.crypto_symbols:
            ...
            if atr_relative < 0.03:
                continue

            # Conditions d'achat
            if not invested and ema_fast > ema_slow and macd > macd_signal and rsi < 70 and open_positions < self.max_open_positions:
